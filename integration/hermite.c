...
gsl_integration_hermite_alloc(const size_t n, const double a, const double b)
{
  ...
  w = calloc(1, sizeof(gsl_integration_hermite_workspace));
  if (w == NULL)
    {
  ...
  w->sdiag = malloc(n * sizeof(double));
  if (w->sdiag == NULL)
    {
      gsl_integration_hermite_free(w);
      GSL_ERROR_VAL ("unable to allocate sdiag", GSL_ENOMEM, 0);
    }

  w->size = n;

  /* compute quadrature weights and nodes */
  hermite_compute(a, b, w);

  return w;
}

void
gsl_integration_hermite_free(gsl_integration_hermite_workspace * w)
{
  if (w->weights)
    free(w->weights);
...
int
gsl_integration_hermite(const gsl_function * func, double * result,
                        gsl_integration_hermite_workspace * w)
{
...
static int
hermite_compute(const double a, const double b, gsl_integration_hermite_workspace * w)
{
  int s;
  const size_t n = w->size;
  const double zemu = 1.772453850905516; /* Gamma(1/2) */
  size_t i;

  /* construct the subdiagonal elements of Jacobi matrix */
  for (i = 1; i <= n; i++)
    {
      w->sdiag[i - 1] = sqrt (0.5 * i);
    }
  ...
  s = imtqlx (n, w->x, w->sdiag, w->weights);
  if (s)
    return s;
   ...
   */
  {
    double slp = 1.0 / sqrt(b);
    size_t k;

    for ( k = 0; k < n; k++ )
      {
        w->x[k] = a + slp * w->x[k];
        w->weights[k] = w->weights[k] * slp;
      }
  }
